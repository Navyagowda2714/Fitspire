...
enum PushUpIssue2: String {
    case correct       = "✅ Perfect Push-Up"
    case notDeepEnough = "❌ Lower Your Chest"
    case hipSagging    = "❌ Keep Hips Level"
    case elbowsFlaring = "❌ Tuck Your Elbows"
    case detecting     = "🔍 Detecting..."
    case notVisible    = "📷 Full Body Not Visible"
}

enum PushUpPhase2 { case up, descending, bottom, ascending }

struct PushUpPostureResult {
    var issue: PushUpIssue2 = .detecting
    var postureScore: Int = 100
    var elbowAngle: Double = 180      // shoulder→elbow→wrist — < 90° at bottom
    var bodyAngle: Double  = 180      // shoulder→hip→ankle — should stay ~170°+
    var shoulderAngle: Double = 45    // elbow flare check
...
final class PushUpViewModel2: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    let session = AVCaptureSession()
    private let sessionQueue = DispatchQueue(label: "fitspire.pushup.session", qos: .userInitiated)
    @Published var bodyPoints: [VNHumanBodyPoseObservation.JointName: CGPoint] = [:]
    @Published var result = PushUpPostureResult()
    @Published var reps = 0
    @Published var phase: PushUpPhase2 = .up
    @Published var phaseText = "Up"; @Published var phaseColor: Color = .white
    // Background-thread-safe copy of phase (avoids data race in captureOutput)
    private var bgPhase: PushUpPhase2 = .up
    ...
    private var hipErrorFrames = 0; private var hadHipError = false
    private var stableFrames = 0; private var lastIssue: PushUpIssue2 = .detecting
    ...
    func resetReps() { DispatchQueue.main.async { self.reps = 0; self.bottomReached = false; self.repStarted = false; self.validBottomFrames = 0; self.validTopFrames = 0; self.hadHipError = false; self.hipErrorFrames = 0 } }
    ...
    private func analyze(_ pts: [VNHumanBodyPoseObservation.JointName: VNRecognizedPoint]) -> PushUpPostureResult {
        var r = PushUpPostureResult()
        let lConf: Float = (pts[.leftShoulder]?.confidence ?? 0) + (pts[.leftElbow]?.confidence ?? 0) + (pts[.leftWrist]?.confidence ?? 0)
        let rConf: Float = (pts[.rightShoulder]?.confidence ?? 0) + (pts[.rightElbow]?.confidence ?? 0) + (pts[.rightWrist]?.confidence ?? 0)
        ...
        for j in [shK, elK, wrK] { guard let p = pts[j], p.confidence > 0.35 else { r.issue = .notVisible; return r } }
        guard let sh = pts[shK]?.location, let el = pts[elK]?.location, let wr = pts[wrK]?.location else {
            r.issue = .notVisible; return r
        }
        r.elbowAngle = angle(sh, el, wr)
        if let hp = pts[hpK], hp.confidence > 0.3, let an = pts[anK], an.confidence > 0.3 {
            r.bodyAngle = angle(sh, hp.location, an.location)
            r.bodyOk    = r.bodyAngle >= 155
        } else { r.bodyOk = true }
        r.elbowOk = r.elbowAngle <= 95 || phase != .bottom
        r.depthOk = r.elbowAngle <= 95 || bgPhase == .up
        var score = 100
        if !r.bodyOk  { score -= 35 }
        r.postureScore = max(score, 0)
        if !r.bodyOk   { r.issue = .hipSagging }
        else if r.elbowAngle > 140 && bgPhase == .bottom { r.issue = .notDeepEnough }
        else { r.issue = .correct }
        return r
    }

    private func updatePhase(_ r: PushUpPostureResult) {
        if bgPhase == .descending || bgPhase == .bottom {
            hipErrorFrames = !r.bodyOk ? hipErrorFrames + 1 : 0
            if hipErrorFrames >= 3 { hadHipError = true }
        }
        if r.elbowAngle < 145 && r.elbowAngle < lastElbow { repStarted = true }
        if repStarted && r.elbowAngle <= 95 {
            validBottomFrames += 1
            if validBottomFrames >= 3 { bottomReached = true }
        } else if r.elbowAngle > 155 && repStarted {
            validTopFrames += 1
            if validTopFrames >= 2 && bottomReached {
                if hadHipError { DispatchQueue.main.async { self.badRepReason = "Keep hips level — core was sagging"; self.showBadRep = true; DispatchQueue.main.asyncAfter(deadline: .now()+2.5) { self.showBadRep = false } } }
                else { DispatchQueue.main.async { self.reps += 1 } }
                bottomReached = false; repStarted = false; validBottomFrames = 0; validTopFrames = 0; hadHipError = false; hipErrorFrames = 0
            }
        } else { validTopFrames = 0 }
        lastElbow = r.elbowAngle
        DispatchQueue.main.async {
            if r.elbowAngle <= 95 { self.phase = .bottom; self.bgPhase = .bottom; self.phaseText = "Deep ✅"; self.phaseColor = .green }
            else if r.elbowAngle < 145 && r.elbowAngle < self.lastElbow { self.phase = .descending; self.bgPhase = .descending; self.phaseText = "Going Down"; self.phaseColor = .yellow }
            else if r.elbowAngle >= 155 { self.phase = .up; self.bgPhase = .up; self.phaseText = "Up"; self.phaseColor = .white }
            else { self.phase = .ascending; self.bgPhase = .ascending; self.phaseText = "Pushing Up"; self.phaseColor = .cyan }
        }
    }

    private func showFormAlert(_ r: PushUpPostureResult) {
        guard bgPhase == .descending || bgPhase == .bottom else { DispatchQueue.main.async { self.showAlert = false }; return }
        var msg: String? = nil
        if !r.bodyOk { msg = "Keep Hips Level — Squeeze Your Core!" }
        DispatchQueue.main.async {
...
// Push-Up Skeleton — per-joint coloring
struct PushUpSkeletonOverlay: View {
    let bodyPoints: [VNHumanBodyPoseObservation.JointName: CGPoint]
    let result: PushUpPostureResult
    var body: some View { puSkeletonBody }
    @ViewBuilder private var puSkeletonBody: some View {
    ...
    private var el: VNHumanBodyPoseObservation.JointName { result.trackedLeftSide ? .leftElbow    : .rightElbow }
    private var wr: VNHumanBodyPoseObservation.JointName { result.trackedLeftSide ? .leftWrist    : .rightWrist }
    ...
    @ViewBuilder private func puBones(geo: GeometryProxy) -> some View {
        puLine(sh, el, geo, ok: result.elbowOk)
        puLine(el, wr, geo, ok: result.elbowOk)
        puLine(sh, hp, geo, ok: result.bodyOk)
        puLine(hp, an, geo, ok: result.bodyOk)
    }
    @ViewBuilder private func puLine(_ j1: VNHumanBodyPoseObservation.JointName, _ j2: VNHumanBodyPoseObservation.JointName, _ geo: GeometryProxy, ok: Bool) -> some View {
    ...
    @ViewBuilder private func puJoints(geo: GeometryProxy) -> some View {
        ForEach([sh, el, wr, hp, an], id: \.self) { j in
            if let pt = bodyPoints[j] {
                Circle().fill(puDotColor(j)).frame(width: 14, height: 14)
                    .overlay(Circle().stroke(Color.white.opacity(0.5), lineWidth: 1.5))
                    .position(x: pt.x*geo.size.width, y: pt.y*geo.size.height)
            }
        }
    }
    private func puDotColor(_ j: VNHumanBodyPoseObservation.JointName) -> Color {
        switch j {
        case .leftShoulder, .rightShoulder, .leftElbow, .rightElbow, .leftWrist, .rightWrist: return result.elbowOk ? .green : .red
        case .leftHip, .rightHip, .leftAnkle, .rightAnkle: return result.bodyOk ? .green : .red
        default: return .white
        }
    }
}

struct PushUpCameraView2: View {
    @StateObject private var vm = PushUpViewModel2()
    var body: some View { puContent }
    @ViewBuilder private var puContent: some View {
        ZStack {
            ExerciseSessionPreview(session: vm.session).ignoresSafeArea()
            PushUpSkeletonOverlay(bodyPoints: vm.bodyPoints, result: vm.result).ignoresSafeArea()
            VStack {
                puTopBar; Spacer()
                if vm.showAlert { FormAlertBanner(message: vm.alertMessage).transition(.move(edge: .top).combined(with: .opacity)) }
                Spacer(); puBottomPanel
            }
    ...
    private var puTopBar: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) { Text("Push-Up AI").font(.title2.bold()).foregroundColor(.white); Text("Real-Time Form Check").font(.caption).foregroundColor(.white.opacity(0.7)) }
            Spacer()
    ...
    private var puBottomPanel: some View {
        VStack(spacing: 18) {
            Text(vm.result.issue.rawValue).font(.title2.bold()).foregroundColor(.white).multilineTextAlignment(.center)
            HStack(spacing: 12) {
                AngleCard(title: "Elbow",     angle: vm.result.elbowAngle, isOk: vm.result.elbowOk,  idealRange: "< 95°")
                AngleCard(title: "Body Line", angle: vm.result.bodyAngle,  isOk: vm.result.bodyOk,   idealRange: "> 155°")
                AngleCard(title: "Depth",     angle: vm.result.elbowAngle, isOk: vm.result.depthOk,  idealRange: "< 95°")
            }
            HStack(spacing: 50) {
